...
```bash
QWEN3_TTS_TEST_MODEL=<talker.gguf> QWEN3_TTS_TEST_VOCODER=<tokenizer.gguf> \
  ctest --test-dir <build dir>          # the whole automated suite
ctest --test-dir <build dir> -R cli     # one test
```

ctest is all of it. Tests whose models or reference dumps are missing report
**Skipped** (exit 77), never failed, so a red run is always a real regression.
`scripts/compare_e2e.py` compares against the PyTorch reference and is run by
hand — it needs the `qwen_tts` Python environment.

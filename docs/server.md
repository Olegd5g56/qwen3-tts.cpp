...
| `QWEN3_TTS_FUSE_WEIGHTS` | auto | Concatenate Q/K/V into one weight and gate/up into another, so a one-token step runs one matmul where it ran three (and two). Built at load time with the separate weights as views into it, so it costs no memory. Bit-identical output. Auto fuses the code predictor everywhere and the talker everywhere **except CUDA**, where one 4096-row matmul lands on a different kernel than three smaller ones and measures 1.6% slower. `1` fuses the talker there too, `0` fuses nothing. A group whose members disagree on type or shape is left alone, which is what a mixed `--tensor-type` model gives. |
